...
def unfold_expected_err(expected_e  # type: ExpectedError
    ...
    elif isinstance(expected_e, str):
...
def needs_binding(f, return_bound=False):
    ...
    if isinstance(f, functools.partial) or inspect.ismethod(f) or is_bound_builtin_method(f):
        ...
        try:
            ...
            if qname == f.__name__:
                ...
                # normal unbound method (since we already eliminated bound ones above with inspect.ismoethod(f))
